...
export async function POST(req: Request) {
  ...
  const { allowed } = rateLimit(key, 5, 60); // 5 submissions per minute per client
  if (!allowed) {
  ...
  const created = createQuoteRequest({
  ...
  createAuditLog(null, 'quote_request.created', created.refNumber);
